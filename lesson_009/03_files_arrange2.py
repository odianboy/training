    ...
    def open_file(self):
        for dirpath, dirname, filenames in os.walk(self.file_name):  # прогулка по всем папка и файлам
            for file_png in filenames:
                ...
                os.rename(full_file_path,'icons_by_year\\'+self.data +'\\'+self.mon +'\\' +file_png)

    def creating_folders_year(self,data):
        name_file = "D:\Питон\skillbox\dz\9\icons_by_year\\"
        self.data = data
        check_file = os.path.exists(name_file + self.data)
        if check_file == False:
            os.mkdir(name_file + self.data)


    def creating_folders_mon(self,mon):
        name_file = "D:\Питон\skillbox\dz\9\icons_by_year\\" + self.data + '\\'
        self.mon = mon
        check_file = os.path.exists(name_file + self.mon)
        if check_file == False:
            os.mkdir(name_file + self.mon + '\\')
